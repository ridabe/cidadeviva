		...
		<?php
				...
				foreach($itemRatting AS $ratValue):
					...
					?>
						<div class="xs-overview-data xs-overview-<?php echo esc_attr($itemRattingStyle)?>">
							...
							<div class="data-rat-label-range"><?php echo esc_html($rattinfDataRat);?> / <?php echo esc_html($rattinfDataRange);?> </div>
			...
			<div class="xs-review-overview-list-right custom-rat">
				<div class="total_overview_rattings">
					<span class="total_rattings_review"> <?php echo esc_html($avgRat);?>  </span> <br/> <strong><?php echo esc_html__('SUPERB!', 'wp-ultimate-review');?></strong>
				</div>
			</div>
		<?php } ?>
	</div>
...
<div class="xs-review-box view-review-list" id="xs-review-box">
	<h3 class="total-reivew-headding"> <?php $numberOfReviews = isset($the_queryTotal->found_posts) ? $the_queryTotal->found_posts : 0; echo $numberOfReviews; ?><?php printf( _nx(' Review', ' Reviews', $numberOfReviews, 'no of reviews', 'wp-ultimate-review')); ?></h3>
   <div class="xs-review-box-item">
   <?php
   
   if( $review_list == 'Yes' ):
   ?>
	   <div class="xs-review-media <?php if(! isset($return_data_overview->overview->ratting->enable) ){ echo esc_attr('review-full');}?>">
		<?php
		$postCount = 1;
				...
				<div class="xs-reviewer-details">
					...
					if( $enable_date == 'Yes' ):
						  if( isset($post->post_date) AND strlen($post->post_date) > 2 ): ?>
								...
								<time datetime="<?php echo esc_attr(get_the_date('c')); ?>" itemprop="datePublished"><?php echo esc_html(get_the_date('F d, Y')); ?></time>
								</div>
					<?php endif;
					 endif;
					 // ratting title
    ...
    <form action="<?php echo esc_url(get_permalink($post->ID));?>" name="xs_review_form_public_data" method="post" id="xs_review_form_public_data">
        ...
        <div class="xs-review-box public-xs-review-box" id="xs-review-box">
			...
			<div class="review_message_show">
				<p> <?php echo esc_html__($_COOKIE['xs_review_message'], 'wp-ultimate-review');  unset($_COOKIE['xs_review_message']); ?></p>
			</div>
			<div class="wur-review-fields">
				...
				if(is_array($this->controls) AND sizeof($this->controls) > 0){
					...
					foreach($this->controls AS $metaKey=>$metaValue):
						...
						if($checkEnable === 'Yes'){
								...
								<div class="xs-review xs-<?php echo esc_attr($inputType);?>" style="<?php echo esc_attr($displayFiled);?>">
									...
									<div class="xs-review-rating-stars text-center">
										<ul id="xs_review_stars">
											...
											<li class="star-li <?php echo esc_attr($review_score_style_input);?>  <?php if($ratting == 1){echo esc_html('selected');}?>" data-value="<?php echo esc_attr( intval($ratting));?>">
										...
										<input type="hidden" id="ratting_review_hidden"  name="<?php echo esc_attr($content_meta_key);?>[<?php echo esc_attr($inputName);?>]" value="1" <?php echo esc_attr($requireSet);?> />
									</div>
									<?php endif;
									if($review_score_style_input == 'slider'):?>
									<div class="xs-review-rating-slider text-center">
										<div class="xs-slidecontainer">
										<input type="range" min="1" max="<?php echo intval($review_score_limit);?>" value="1"  name="<?php echo esc_attr($content_meta_key);?>[<?php echo esc_attr($inputName);?>]" class="xs-slider-range" id="xs_review_range">
							...
							?>
							<div class="xs-review xs-<?php echo esc_attr($inputType);?>" style="<?php echo esc_html($displayFiled);?>">
								<label for="<?php echo esc_attr($inputId);?>" > <?php echo esc_html($inputTitle)?>
									<select name="<?php echo esc_attr($content_meta_key);?>[<?php echo esc_attr($inputName);?>]" id="<?php echo esc_attr($inputId);?>" class="widefat <?php echo esc_attr( $inputClass );?>" <?php echo esc_attr($requireSet);?> >
										<?php
							...
							<div class="xs-review xs-<?php echo esc_attr( $inputType );?>" style="<?php echo esc_attr($displayFiled);?>">
								...
								?>
									<label for="<?php echo esc_attr($optionsKey);?>">
										<input type="<?php echo esc_attr($inputType);?>" id="<?php echo esc_attr($optionsKey);?>" class="widefat <?php echo esc_attr( $inputClass );?>" name="<?php echo esc_attr($content_meta_key);?>[<?php echo esc_attr($inputName);?>]" value="<?php echo esc_html( $optionsKey ) ?>" <?php echo (isset($optionsKey) && $optionsKey == $metaData) ? 'checked' : '' ?> <?php echo esc_attr($requireSet);?> />
							...
							<div class="xs-review xs-<?php echo esc_attr($inputType);?>" style="<?php echo esc_attr($displayFiled);?>">
							<!-- <label for="<?php echo esc_attr($inputId);?>" > <?php echo esc_html($inputTitle)?> -->
									<textarea id="<?php echo esc_attr($inputId);?>" class="widefat <?php echo esc_attr( $inputClass );?>" name="<?php echo esc_attr($content_meta_key);?>[<?php echo esc_attr($inputName);?>]" <?php echo esc_attr($requireSet);?> placeholder="<?php echo esc_attr($inputTitle);?>" ><?php echo esc_attr($metaData); ?></textarea>
								<!--</label>-->
							</div> 
							<?php
							}else{
							...
							<div class="xs-review xs-<?php echo esc_attr($inputType);?>" style="<?php echo esc_attr($displayFiled);?>">
							<!-- <label for="<?php echo esc_attr($inputId);?>" > <?php echo esc_html($inputTitle)?> -->
									<input type="<?php echo esc_attr($inputType);?>" placeholder="<?php echo esc_html($inputTitle);?>" id="<?php echo esc_attr($inputId);?>" class="widefat <?php echo esc_attr( $inputClass );?>" name="<?php echo esc_attr($content_meta_key);?>[<?php echo esc_attr($inputName);?>]" value="<?php echo esc_attr($metaData); ?>" <?php echo esc_attr($requireSet);?> />
								<!--</label>-->
							</div>
							<?php }
						}
				...
				?>
				<input type="hidden" value="<?php echo esc_attr($this->getPostId);?>" name="<?php echo esc_attr($content_meta_key);?>[xs_post_id]" />
				<input type="hidden" value="<?php echo esc_attr($this->getPostType);?>" name="<?php echo esc_attr($content_meta_key);?>[xs_post_type]" />
				<input type="hidden" value="<?php echo get_current_user_id();?>" name="<?php echo esc_attr($content_meta_key);?>[xs_post_author]" />
