...
Class Cpt{
     ...
     */
	 public function wur_meta_box_init(){
		...
		if(is_array($page_enable) AND sizeof($page_enable) > 0):
	...
	public function wur_meta_box_data_save( $post_id, $post ){
		...
		if(!empty($post_id) AND is_object($post)) {
			...
			if( $enable_review == 'Yes'):
				...
				if( in_array($post->post_type, $page_enable) ):
					...
					$metaOverviewData = Settings::sanitize($metaOverviewData);
					if(is_array($metaOverviewData) AND sizeof($metaOverviewData) > 0):
						...
						update_post_meta( $post_id, $overview_setting_optionKey, Settings::_encode_json($metaOverviewData));
					endif;
